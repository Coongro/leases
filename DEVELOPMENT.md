...
- **Trigger name**: `trg_leases_{descriptive_name}`
- **Function name**: `trg_leases_{descriptive_name}_fn`

Where `leases` is your plugin's moduleId (pluginId with hyphens replaced by underscores).
...
CREATE OR REPLACE FUNCTION "trg_leases_audit_log_fn"()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER "trg_leases_audit_log"
AFTER INSERT OR UPDATE ON "module_leases_items"
FOR EACH ROW
EXECUTE FUNCTION "trg_leases_audit_log_fn"();
```

> **Important**: The system automatically **DISABLE**s all `trg_leases_*` triggers when the plugin
