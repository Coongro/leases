/**
 * Resultado — datos y estado (generado por el Builder de Vistas).
 *
...
export function useResultadoView() {
    ...
    try {
      ...
      const data = byBlock
        ...
        : customHandlers.loadData
          ...
          : await actions.execute<any[]>('leases.billing.list');
  ...
  useEffect(() => {
    const offs = [
      'leases.billing.create',
      'leases.billing.update',
      'leases.billing.delete',
      'leases.billing.restore',
    ].map((id) =>
      events.on(id, () => {
        void load();
      })
  ...
  const COLUMNS: {
    ...
    { key: 'rentCollected', label: 'Alquiler cobrado', display: 'mono', format: 'money' },
    { key: 'unpaidRent', label: 'Sin cobrar', display: 'mono', format: 'money' },
    { key: 'expenses', label: 'Gastos a tu cargo', display: 'mono', format: 'money' },
    { key: 'net', label: 'Neto', display: 'mono', format: 'money' },
    { key: 'expenseRatio', label: '% de la renta', display: 'mono', suffix: '%' },
  ];
  // columnas OCULTAS: alimentan el detalle de la fila expandible
  const HIDDEN_COLUMNS: typeof COLUMNS = [
    { key: 'adminFee', label: 'Honorarios de administración', display: 'mono', format: 'money' },
  ];
  const cellValue = (
    row: any,
  ...
  const confirmDelete = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await actions.execute('leases.billing.delete', { id: pendingDelete.id });
