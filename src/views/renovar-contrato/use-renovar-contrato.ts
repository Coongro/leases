/**
 * Renovar contrato — datos y estado (generado por el Builder de Vistas).
 *
...
export function useRenovarContratoView() {
  const {
    toast,
    views: { closeDialog },
  } = usePlugin();
  ...
  const validate = useCallback((): Record<string, string> => {
    ...
    if (
      ...
      errs['endDate'] = '«Hasta» es requerido';
    if (
      values['rentAmount'] === null ||
      values['rentAmount'] === undefined ||
      values['rentAmount'] === '' ||
      values['rentAmount'] === false
    )
      errs['rentAmount'] = '«Alquiler de la renovación» es requerido';
    return errs;
  }, [values]);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      toast.success('Contrato renovado', 'Se creó el contrato nuevo y el anterior quedó cerrado.');
      setEditingId(null);
      setValues({
        startDate: null,
        endDate: null,
