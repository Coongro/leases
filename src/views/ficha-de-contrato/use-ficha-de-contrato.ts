/**
 * Ficha de contrato — datos y estado (generado por el Builder de Vistas).
 *
...
export function useFichaDeContratoView() {
  ...
  const useTable1 = () => {
    ...
    const load = useCallback(async () => {
      ...
      try {
        ...
        const byBlock = customHandlers.loadDataFor?.['tbl_ajustes'];
        const data = byBlock
          ? await byBlock(ctx)
    ...
    const COLUMNS: {
      ...
      { key: 'rate', label: 'Variación', display: 'mono', suffix: '%' },
      { key: 'previous_rent', label: 'Anterior', display: 'mono', format: 'money' },
      { key: 'new_rent', label: 'Nuevo', display: 'mono', format: 'money' },
      {
        key: 'status',
        label: 'Estado',
        display: 'pill',
        values: [
          ...
          { value: 'cancelled', label: 'Cancelada', tone: 'outline' },
        ],
      },
    ];
    const cellValue = (
  ...
  const useTable2 = () => {
    ...
    const COLUMNS: {
      ...
      { key: 'total', label: 'A pagar', display: 'mono', format: 'money' },
  ...
  const useTable3 = () => {
    ...
    const COLUMNS: {
      ...
      { key: 'label', label: 'Concepto' },
      {
        ...
        display: 'pill',
        values: [
          { value: 'abl', label: 'ABL / inmobiliario', tone: 'neutral', icon: 'Landmark' },
          { value: 'servicio', label: 'Servicio', tone: 'neutral', icon: 'Droplets' },
          { value: 'expensas', label: 'Expensas', tone: 'neutral', icon: 'Building2' },
          { value: 'descuento', label: 'Descuento', tone: 'success', icon: 'TicketPercent' },
          { value: 'otro', label: 'Otro', tone: 'outline', icon: 'Receipt' },
        ],
      },
      { key: 'valid_to', label: 'Hasta', emptyLabel: 'Sin vencimiento' },
      { key: 'amount', label: 'Por mes', display: 'mono', format: 'money' },
    ];
    // el subtítulo se muda bajo el título: fuera de las columnas propias
    const SUB_COL = COLUMNS.find((c) => c.key === 'type');
    const ITEM_COLS = COLUMNS.filter((c) => c.key !== 'type');
    const cellValue = (
      row: any,
      c: { key: string; ref?: string; refDisplay?: string; refPath?: string; ref2?: string }
    ) => {
