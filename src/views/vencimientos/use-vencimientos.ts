/**
 * Vencimientos — datos y estado (generado por el Builder de Vistas).
 *
...
export function useVencimientosView() {
    ...
    try {
      ...
      const data = byBlock
        ...
        : customHandlers.loadData
          ...
          : await actions.execute<any[]>('leases.expiries.list');
  ...
  useEffect(() => {
    const offs = [
      'leases.expiries.create',
      'leases.expiries.update',
      'leases.expiries.delete',
      'leases.expiries.restore',
    ].map((id) =>
      events.on(id, () => {
        void load();
      })
  ...
  const COLUMNS: {
  ...
  }[] = [
    {
      ...
      display: 'pill',
      values: [
        { value: 'vencido', label: 'Vencido', tone: 'danger', icon: 'TriangleAlert' },
        { value: 'por_vencer', label: 'Por vencer', tone: 'warning', icon: 'Clock' },
      ],
    },
    {
      key: 'kind',
      ...
      display: 'pill',
      values: [
        { value: 'certificado', label: 'Certificado', tone: 'outline', icon: 'ShieldCheck' },
        { value: 'contrato', label: 'Contrato', tone: 'outline', icon: 'FileText' },
        { value: 'garantia', label: 'Garantía', tone: 'outline', icon: 'Umbrella' },
      ],
    },
    { key: 'label', label: 'Detalle' },
    { key: 'message', label: 'Cuándo' },
    { key: 'expires_at', label: 'Vence', format: 'date' },
  ];
  // el subtítulo se muda bajo el título: fuera de las columnas propias
  const SUB_COL = COLUMNS.find((c) => c.key === 'message');
  const ITEM_COLS = COLUMNS.filter((c) => c.key !== 'message');
  const cellValue = (
    row: any,
  ...
  const confirmDelete = useCallback(async () => {
    ...
    setDeleting(true);
    try {
      await actions.execute('leases.expiries.delete', { id: pendingDelete.id });
