/**
 * Concepto del contrato — datos y estado (generado por el Builder de Vistas).
 *
...
export function useConceptoDelContratoView() {
  const {
    toast,
    views: { closeDialog },
  } = usePlugin();
  ...
  const PARENT_REF_FIELD: Record<string, string> = { 'leases.contracts': 'lease_id' };
  useEffect(() => {
  ...
  useEffect(() => {
    void Promise.all([
      actions
        ...
        .then((r) => {
          if (mounted.current)
            setRefOptions((o: any) => ({ ...o, lease_id: Array.isArray(r) ? r : [] }));
        })
        .catch(() => {}),
    ]);
    // deps intencionalmente fijas: el efecto corre una sola vez
  ...
  const validate = useCallback((): Record<string, string> => {
    ...
    if (
      values['lease_id'] === null ||
      values['lease_id'] === undefined ||
      values['lease_id'] === '' ||
      values['lease_id'] === false
    )
      errs['lease_id'] = '«Contrato» es requerido';
    if (
      values['type'] === null ||
    ...
    if (
      ...
      errs['label'] = '«Como aparece en la factura» es requerido';
    if (
      values['amount'] === null ||
  ...
  const submit = useCallback(async () => {
    ...
    try {
      if (customHandlers.onSubmit) {
        ...
        await actions.execute('leases.charges.update', { id: editingId, data: values });
      } else {
        await actions.execute('leases.charges.create', { data: values });
      }
