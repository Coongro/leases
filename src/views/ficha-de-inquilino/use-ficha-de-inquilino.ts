/**
 * Ficha de inquilino — datos y estado (generado por el Builder de Vistas).
 *
...
export function useFichaDeInquilinoView() {
  ...
  const useTable1 = () => {
    ...
    const load = useCallback(async () => {
      ...
      try {
        ...
        const byBlock = customHandlers.loadDataFor?.['tbl_contratos'];
        const data = byBlock
          ? await byBlock(ctx)
    ...
    const COLUMNS: {
      ...
      { key: 'rent_amount', label: 'Alquiler', display: 'mono', format: 'money' },
      {
        key: 'state',
        label: 'Estado',
        display: 'pill',
